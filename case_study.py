...
country_total_price = df.groupby(["PRICE", "COUNTRY"]).size().reset_index(name='Sales_Count') # df series di => reset index data frame e çevirdi
...
type(total_earnings_by_country)
...
average_gains = df.groupby(["COUNTRY", "SOURCE", "SEX", "AGE"])["PRICE"].mean()
